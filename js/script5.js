...
/*$('#sliderr').vegas({
		...
		transition: 'fade2',//切り替わりのアニメーション。http://vegas.jaysalvat.com/documentation/transitions/参照。fade、fade2、slideLeft、slideLeft2、sliderright、sliderright2、slideUp、slideUp2、slideDown、slideDown2、zoomIn、zoomIn2、zoomOut、zoomOut2、swirlLeft、swirlLeft2、swirlRight、swirlRight2、burnburn2、blurblur2、flash、flash2が設定可能。
...
// TextRandomAnime5にappearRandomtextというクラス名を付ける定義
function TextRandomAnime5Control() {
	$('.TextRandomAnime5').each(function () {
...
//テキストのカウントアップの設定
var bar = new ProgressBar.Line(splash5_text, {//id名を指定
...
//=====ここからローディングエリア（splash5エリア）を0.8秒でフェードアウトした後に動かしたいJSをまとめる
	$("#splash5").delay(500).fadeOut(800,function(){//#splash5エリアをフェードアウトした後にアニメーションを実行
    PageTopCheck();// スクロール途中からリンクボタンの形状が変化の関数を呼ぶ
    delayScrollAnime();//印象編 4-12 順番に現れる（CSS x jQuery）の関数を呼ぶ
	TextRandomAnime5Control();//印象編 8-1 テキストがバラバラに出現の関数を呼ぶ
    });
//=====ここまでローディングエリア（splash5エリア）を0.8秒でフェードアウトした後に動かしたいJSをまとめる
